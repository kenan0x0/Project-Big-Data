...
# Empty list to check how many files were installed. Declared empty and will be emptied each time the software executes.
number_installed = []
...
def scrape_flask(username, password, teamname, mat_trn, brow, date, date1, mode):
    try:
        ...
        if brow == "Firefox":
            ...
            browser = webdriver.Firefox(options=options_ff, firefox_profile=profile, executable_path="C:\Project\drivers\gecko\geckodriver.exe")
            ...
            prefs = {"download.default_directory" : "C:\\Project\\Stats"}
            chromeOptions.add_experimental_option("prefs",prefs)
            chromeOptions.add_argument("--window-size=1400,800")
            # chrome_options = GCOptions()
            if mode == "y":
                chromeOptions.add_argument("--headless")
                browser = webdriver.Chrome(executable_path="C:\Project\drivers\chrome\chromedriver.exe", chrome_options=chromeOptions)
            else:
                browser = webdriver.Chrome(executable_path="C:\Project\drivers\chrome\chromedriver.exe", chrome_options=chromeOptions)
            ...
            browser = Edge(executable_path="C:\Project\drivers\edge\msedgedriver.exe", options=edgeOpts)
            browser.set_window_size(1400, 800)



        # LOGGING INTO THE GENGEE WEBSITE
        ...
        # SELECT BETWEEN MATCH AND TRAIN SPREADSHEETS
        ...
        # SELECT THE TEAM NAME (CAN BE LEFT BLANK)
        ...
        # DATE PICKER. COMPLICATED LOGIC!
        # Split the date inputted by the user into year, month and day. Months' numerical values are replaced with the months' actual names in order to fit GenGee's style.
        year = date[0:4]
        month = date[5:7]
        if month == '01':
            month = "January"
...
# REMOVE IF DOESN'T WORK!!! WORK IN PROGRESS. MIGHT RUIN THE PROGRAM
        year1 = date1[0:4]
        month1 = date1[5:7]
        if month1 == '01':
            month1 = "January"
...
# REMOVE IF DOESN'T WORK!!! WORK IN PROGRESS. MIGHT RUIN THE PROGRAM
        ...
        # Download all in case date isn't given
        if date == "":
            ...
            for x in range(dec):
                ...
                try:
                    ...
                    time.sleep(1)
                    # To circumvent StaleElement Execptions
                    all_files = browser.find_elements_by_class_name("journal-download-excel_wrapper")
                except NoSuchElementException:
                    pass

        # Download the spreadsheets of the given date
        ...
        # Logout of the account to avoid double login and suspicious activity alerts
...
# Function resposible for returning the number of downloaded files in each program run.
def installed_files():
    return number_installed

# Function resposible for clearing the list for the next software run.
def clear_lis():
